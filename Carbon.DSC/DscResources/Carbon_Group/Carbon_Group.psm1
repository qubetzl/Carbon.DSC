...
function Resolve-MembershipChanges()
{
    [CmdletBinding()]
    [OutputType([hashtable])]
    param (
    ...
    $intersection = @($MemberNames | Where-Object {$_ -In $CurrentMemberNames})
    $extraCurrentMembers = @($CurrentMemberNames | Where-Object {$_ -NotIn $MemberNames})
    $extraMembers = @($MemberNames | Where-Object {$_ -NotIn $CurrentMemberNames})

    $membersToAdd = $null;
    $membersToRemove = $null;
    if( $EnsureMembers -eq "Exact" )
    {
        $membersToAdd = $extraMembers
        $membersToRemove = $extraCurrentMembers
    }
    elseif( $EnsureMembers -eq 'Present'  )
    {
        $membersToAdd = $extraMembers
        $membersToRemove = @()
    }
